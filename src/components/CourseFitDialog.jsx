...
import { LuX, LuCircleAlert, LuCircleCheck, LuSend, LuLoader, LuSparkles } from "react-icons/lu";
...
function Field({ label, name, error, optional, children }) {
  return (
    <div>
      <label htmlFor={name} className="mb-2 flex items-baseline gap-2 text-xs font-semibold text-zinc-300">
        {label}
        {optional ? (
...
export default function CourseFitDialog({ open, onClose }) {
  ...
  async function handleSubmit(e) {
    ...
    const payload = {
      ...
      source: "course-fit-popup",
      submittedAt: new Date().toISOString(),
    };

    // TODO: POST this to /api/lead. Logging for now, as asked.
    console.log("Course fit check →", payload);
      ...
      {open ? (
        <div className="fixed inset-0 z-[70] flex items-end justify-center p-3 sm:items-center sm:p-6">
          ...
          <motion.div
            ...
            className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-white/[0.12] bg-ink-900 shadow-lift"
                ...
                {status === "sent" ? (
                  ...
                  >
                    ...
                    <p className="mt-2.5 text-sm leading-relaxed text-zinc-400">
                      Three quick details and a counsellor will call you back. We will
                      tell you which of the three courses actually suits your background
                    ...
                    <form onSubmit={handleSubmit} noValidate className="mt-7 space-y-4">
                      ...
                      <Field label="Course you're interested in" name="course" error={errors.course}>
                        <select
                          id="course"
                          name="course"
